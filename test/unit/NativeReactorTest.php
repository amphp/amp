...
class NativeReactorTest extends PHPUnit_Framework_TestCase {
    ...
    function testScheduleReturnsWatcherId() {
        $reactor = new NativeReactor;
        $watcherId = $reactor->schedule(function(){}, $interval = 1);
        $this->assertSame(1, $watcherId);
    }
    ...
    function testImmediatelyAlarmAssignmentWhileAlreadyRunning() {
        ...
        $counter = 0;
        $nestedCounter = 0;
        $watcherId = $reactor->schedule(function() use ($reactor, &$counter, &$nestedCounter) {
            $counter++;
            if ($counter === 1) {
    ...
    function testReactorDoesntSwallowExceptionOnRecurringScheduledAlarm() {
        ...
        $reactor->schedule(function(){ throw new Exception('test'); }, $interval = 0);
